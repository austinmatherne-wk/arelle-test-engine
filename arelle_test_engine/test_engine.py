...
from arelle_test_engine.constraint_result import ConstraintResult
...
def _validate(session: Session, testcase: Testcase) -> Iterable[ActualError]:
    assert session._cntlr is not None
    if testcase.expected_instance_count is not None:
        actual_instance_count = len([
            model for model in session._cntlr.modelManager.loadedModelXbrls
        ...
        if actual_instance_count != testcase.expected_instance_count:
            yield ActualError(
                code="testEngine:expectedInstanceCount",
...
class TestEngine:
    ...
    def __init__(self, options: TestEngineOptions) -> None:
    ...
    def _get_diff(self, constraint_set: ConstraintSet, actual_error_counts: dict[tuple[str, ErrorLevel], int]) -> dict[tuple[str | QName, ErrorLevel], int]:
        ...
        compare_context = CompareContext(
            ...
            local_name_map=arelle_test_engine.constants.errMsgNamespaceLocalNameMap,
            prefix_namespace_uri_map=arelle_test_engine.constants.errMsgPrefixNS,
        )
    ...
    def _run_testcase(
        ...
        with Session() as session:
            ...
            for engine_error in _validate(session, testcase):
                actual_errors.append(engine_error)
    ...
    def run(
        ...
        if self._test_engine_options.parallel:
            ...
            results = self._run_testcases_in_parallel(testcases, processes, result_callback, ui_thread_queue)
        else:
            print("Running in series...")
            results = self._run_testcases_in_series(testcases, result_callback, ui_thread_queue)
