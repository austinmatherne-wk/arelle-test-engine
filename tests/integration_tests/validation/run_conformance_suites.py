...
from tests.integration_tests.validation.download_assets import (
    download_assets,
    verify_assets,
)
from tests.integration_tests.validation.validation_util import (
    CONFORMANCE_SUITE_EXPECTED_RESOURCES_DIRECTORY,
    get_conformance_suite_test_results,
...
def _get_conformance_suite_names(select_option: str) -> tuple[ConformanceSuiteConfig, ...]:
    ...
    if select_option == SELECT_PUBLIC:
...
def run_conformance_suites(
    ...
    for config in conformance_suite_configs:
        ...
        if shard:
            ...
            full_run = set(shards) == set(range(config_shard_count))
